...
// Fetch vendor trends data
export const fetchVendorTrends = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/vendor/trends`, {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch vendor trends data');
    }
